...
- f33e00e: Admin improvements: feedback permissions, bulk operations, and UI refinements
...
- 686b101: feat(claims): add document status indicators and warnings
...
- ## Патч-релиз v0.4.1
  ...
  **Workflows: Prune Releases**
  - Добавлен подробный вывод для шага очистки старых релизов
  - Показывает общее количество, список всех релизов, удаляемые релизы
  - Улучшенная отладка с emoji и нумерованными списками

  **Админка: Автогенерация Slug для новостей**
  - ✨ Кнопка автогенерации slug из заголовка
  - Транслитерация кириллицы → латиница [a-z0-9-]
  - Автоматическая проверка дубликатов с добавлением суффикса (-1, -2)
  - Fallback на YYYY-MM-DD если заголовок пустой
  - Валидация уникальности с исключением текущей новости

  **Организация проекта**
  - Все release notes перемещены в `docs/releases/`
  - Документация организована в `docs/`
  - Корень проекта очищен от лишних MD файлов

  ### 🔧 Технические детали
  - Добавлена tRPC процедура `news.generateSlug`
  - UI обновлен в обеих формах (create/edit)
  - Bash скрипт для prune с полным логированием
  - Safety limit: максимум 100 итераций при поиске уникального slug
