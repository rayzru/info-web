...
import { and, count, desc, eq, sql, or, inArray, lte, gte, ne, isNull } from "drizzle-orm";
import { z } from "zod";
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const newStatus = isAdmin
        ? existing.status
        : (existing.status === "published" ? "pending" : existing.status);
  ...
  list: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (buildingId) {
        conditions.push(
          or(
            eq(publications.buildingId, buildingId),
            sql`${publications.buildingId} IS NULL`
          )!
        );
      }
