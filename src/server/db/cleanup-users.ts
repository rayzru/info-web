...
async function cleanupDatabase() {
  ...
  logger.info("⚠️  This will DELETE all user content while preserving structural data!\n");
  ...
  try {
    ...
    logger.info("📝 Deleting publications...");
    ...
    // 8. Delete audit logs
    logger.info("📊 Deleting audit logs...");
    await truncateIfExists("info_web_audit_log");

    // 9. Delete media
    logger.info("🖼️ Deleting media...");
    ...
    logger.info("🗑️ Deleting deletion requests...");
    await truncateIfExists("info_web_deletion_request");

    // 12. Delete user-property relationships
    logger.info("🔗 Deleting user-property relationships...");
    ...
    logger.info("\n📋 Preserved data:");
    logger.info("  • Buildings, entrances, floors, apartments");
    logger.info("  • Parkings and parking spots");
    logger.info("  • Organizations");
    logger.info("  • Directory entries (справочник)");
    logger.info("  • Knowledge base articles (howtos)");
    logger.info("  • Contact groups and properties");
