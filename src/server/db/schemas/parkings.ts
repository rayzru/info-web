...
export const parkingFloors = createTable("parking_floor", {
  id: varchar("id", { length: 255 }).notNull().primaryKey().$defaultFn(() => crypto.randomUUID()),
  parkingId: varchar("parking_id", { length: 255 }).notNull().references(() => parkings.id, { onDelete: "cascade" }),
...
export const parkingSpots = createTable("parking_spot", {
  id: varchar("id", { length: 255 }).notNull().primaryKey().$defaultFn(() => crypto.randomUUID()),
  floorId: varchar("floor_id", { length: 255 }).notNull().references(() => parkingFloors.id, { onDelete: "cascade" }),
...
export const parkingsRelations = relations(parkings, ({ one, many }) => ({
  building: one(buildings, { fields: [parkings.buildingId], references: [buildings.id] }),
  floors: many(parkingFloors),
}));

export const parkingFloorsRelations = relations(parkingFloors, ({ one, many }) => ({
  parking: one(parkings, { fields: [parkingFloors.parkingId], references: [parkings.id] }),
  spots: many(parkingSpots),
}));

export const parkingSpotsRelations = relations(parkingSpots, ({ one }) => ({
  floor: one(parkingFloors, { fields: [parkingSpots.floorId], references: [parkingFloors.id] }),
}));
