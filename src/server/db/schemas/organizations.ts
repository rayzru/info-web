...
export const organizationToTags = createTable("organization_to_tag", {
  organizationId: varchar("organization_id", { length: 255 }).notNull().references(() => organizations.id, { onDelete: "cascade" }),
  tagId: varchar("tag_id", { length: 255 }).notNull().references(() => organizationTags.id, { onDelete: "cascade" }),
}, (table) => ({
...
export const organizationsRelations = relations(organizations, ({ one, many }) => ({
  building: one(buildings, { fields: [organizations.buildingId], references: [buildings.id] }),
  tags: many(organizationToTags),
}));

export const organizationToTagsRelations = relations(organizationToTags, ({ one }) => ({
  organization: one(organizations, { fields: [organizationToTags.organizationId], references: [organizations.id] }),
  tag: one(organizationTags, { fields: [organizationToTags.tagId], references: [organizationTags.id] }),
}));
